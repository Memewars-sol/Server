...
namespace Memewars.RealtimeNetworking.Server
{
    class Database
    {
        ...
        private static void AddClanXP(NpgsqlConnection connection, long clan_id, int xp)
        {
            int haveXp = 0;
            int level = 0;
            ...
            for (int i = battles.Count - 1; i >= 0; i--)
            {
                ...
                if (battles[i].battle.end)
                {
                    ...
                    if (battleType == Data.BattleType.war)
                    {
                        ...
                        if (warId > 0)
                        {
                            query = String.Format("INSERT INTO clan_war_attacks (war_id, attacker_id, defender_id, stars, looted_gold, looted_elixir, looted_dark_elixir, replay_path) VALUES({0}, {1}, {2}, {3}, {4}, {5}, {6}, '{7}') RETURNING id", warId, attacker_id, defender_id, stars, lootedGold, lootedElixir, lootedDark, path.Replace("\\", "\\\\"));
                            using (NpgsqlCommand command = new NpgsqlCommand(query, connection))
                            {
        ...
        private static int _CreateClanAsync(long account_id, string name, int minTrophies, int minTownhallLevel, int pattern, int background, string patternColor, string backgroundColor, int joinType)
        {
            ...
            using (NpgsqlConnection connection = GetDbConnection())
            {
                ...
                if (response == 0)
                {
                    ...
                    {
                        long clan_id = 0;
                        query = String.Format("INSERT INTO clans (name, min_trophies, min_townhall_level, pattern, background, pattern_color, background_color, join_type) VALUES('{0}', {1}, {2}, {3}, {4}, '{5}', '{6}', {7}) RETURNING id", name, minTrophies, minTownhallLevel, pattern, background, patternColor, backgroundColor, joinType);
                        using (NpgsqlCommand command = new NpgsqlCommand(query, connection))
                        {
                            clan_id = (long)command.ExecuteScalar();
                        }
                        if (clan_id > 0)
                        {
        ...
        private static Data.ClansList _GetClansAsync(int page, long account_id = 0, long clan_id = 0)
        {
            ...
            using (NpgsqlConnection connection = GetDbConnection())
            {
                if (page <= 0)
                {
                    ...
                    {
                        using (NpgsqlCommand command = new NpgsqlCommand(String.Format("SELECT clan_id FROM accounts WHERE id = {0};", account_id), connection))
                        {
                            using (NpgsqlDataReader reader = command.ExecuteReader())
                            {
                                if (reader.HasRows)
        ...
        private static int GetClanRank(NpgsqlConnection connection, long clan_id)
        {
            int rank = 0;
            string query = String.Format("SELECT id, rank FROM (SELECT id, ROW_NUMBER() OVER(ORDER BY trophies DESC) AS 'rank' FROM clans) AS ranks WHERE id = {0}", clan_id);
            using (NpgsqlCommand command = new NpgsqlCommand(query, connection))
            {
                using (NpgsqlDataReader reader = command.ExecuteReader())
                {
        ...
        private static Data.Clan GetClan(NpgsqlConnection connection, long clan_id)
        {
            Data.Clan clan = null;
        ...
        private static List<Data.ClanMember> GetClanMembers(NpgsqlConnection connection, long clan_id)
        {
        ...
        private static Data.Clan GetClanByAccountId(NpgsqlConnection connection, long account_id)
        {
            Data.Clan clan = null;
            long clan_id = 0;
            string query = String.Format("SELECT clan_id FROM accounts WHERE id = {0};", account_id);
        ...
        private static void StartClanWar(NpgsqlConnection connection, Data.ClanWarSearch search1_id, Data.ClanWarSearch search2_id)
        {
            ...
            {
                long id = 0;
                query = String.Format("INSERT INTO clan_wars (clan_1_id, clan_2_id) VALUES({0}, {1}) RETURNING id", search1_id.clan, search2_id.clan);
                using (NpgsqlCommand command = new NpgsqlCommand(query, connection))
                {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (valid)
                                    {
                                        ...
                                        query = String.Format("INSERT INTO clan_wars_search (clan_id, account_id) VALUES({0}, {1}) RETURNING id;", clan_id, account_id);
                                        using (NpgsqlCommand command = new NpgsqlCommand(query, connection))
                                        {
                                            id = (long)command.ExecuteScalar();
                                        }
